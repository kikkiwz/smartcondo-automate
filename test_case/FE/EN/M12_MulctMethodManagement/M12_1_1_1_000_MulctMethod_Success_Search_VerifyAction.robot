...
Verify API onload and mapping data
    ...
    ${checkNodata}=    Run Keyword If    '${rowcountSearch}'=='0'    Wait Until Contains Element Text Should Be    ${MULCTMETHOD_SEARCH_LBL_RESULT_NODATA_LOCATOR}    ${DEFAULT_LBL_NODATA_EN}    #Nodata
    ...    ELSE IF    '${rowcountSearch}'!='0'    Check Value Table Search MulctMethod    ${DEFAULT_EN}     ${rowcountSearch}    ${dataSearch}
...
Verify Action : click search button [Search Status]
    ...
    #Status
    Select Dropdown    ${MULCTMETHOD_SEARCH_DDL_STATUS_LOCATOR}    ${MULCTMETHOD_SEARCH_DDL_STATUS_LIST_LOCATOR}    ${MULCTMETHOD_SEARCH_DDL_STATUS_CLASS_LOCATOR}    ${MULCTMETHOD_STATUS_EN_VALUE1}
    Sleep    0.5s
    #Click Search
    Click Element Page    ${MULCTMETHOD_SEARCH_BTN_SEARCH_LOCATOR}
...
Verify Action : click delete button
    ...
    Check Popup Delete EN    ${MULCTMETHOD_BTN_DELETE_LOCATOR}    ${MulctMethod}
...
################### Generic Test Case Teardown ###################
    [Teardown]    Generic Test Case Teardown MulctMethod2 EN
