...
Verify API Onload and mapping data after click choose property	
	...
	${checkNodata}=    Run Keyword If    '${rowcountSearchRoomManagement}'!='0'    Check Value RoomList    ${DEFAULT_EN}     ${rowcountSearchRoomManagement}    ${dataSearchRoomManagement}
...
Verify Action : click search button (Search status dropdown)
	...
	#Status
	Select Dropdown    ${ROOMMANAGEMENT_SEARCH_DDL_STATUS_LOCATOR}    ${ROOMMANAGEMENT_SEARCH_DDL_STATUS_LIST_LOCATOR}    ${ROOMMANAGEMENT_SEARCH_DDL_STATUS_CLASS_LOCATOR}    ${ROOMMANAGEMENT_STATUS_EN_VALUE3}
	Sleep    0.5s
	#Click Search
	...
	#====== Verify DB ==========
    #Inquiry for verify DB  
    ${resultSearchRoomManagement}=    Run keyword And Continue On Failure    Request Verify DB Check Data Search Check Room Management    ${MYSQL_TYPE_CHECKROOMMANAGEMENT_SEARCH_STATUS}    null    null    null    null    ${DB_TB_CHECKROOMMANAGEMENT_FIELD_FLOOR},${DB_TB_CHECKROOMMANAGEMENT_FIELD_ROOMNO} ${ORDERBY_ASC}
	# Log To Console    resultSearchRoomManagement::${resultSearchRoomManagement} 
...
Verify Action : click Room button and click X button
	...
	#Label: ห้อง XXX
	Wait Until Contains Element Text Should Be    ${ROOMMANAGEMENT_SEARCH_LBL_HEADERPOPUP_ROOMA101_LOCATOR}    ${ROOMMANAGEMENT_SEARCH_LBL_HEADERPOPUP_ROOMA101_EN}

	#Label: ประเภทห้อง
	Wait Until Contains Element Text Should Be    ${ROOMMANAGEMENT_SEARCH_LBL_ROOMTYPE_LOCATOR}    ${ROOMMANAGEMENT_SEARCH_LBL_ROOMTYPE_EN}


	#Label: สถานะห้อง
	Wait Until Contains Element Text Should Be    ${ROOMMANAGEMENT_SEARCH_LBL_ROOMSTATUS_LOCATOR}    ${ROOMMANAGEMENT_SEARCH_LBL_ROOMSTATUS_EN}


	#Label: ห้องโอนแล้ว
	Wait Until Contains Element Text Should Be    ${ROOMMANAGEMENT_SEARCH_LBL_TRANSFERRED_LOCATOR}    ${ROOMMANAGEMENT_SEARCH_LBL_TRANSFERRED_EN}


	#Label: ดูข้อมูลห้องพัก
	Wait Until Contains Element Text Should Be    ${ROOMMANAGEMENT_SEARCH_LBL_ROOMDETAILS_LOCATOR}    ${ROOMMANAGEMENT_SEARCH_LBL_ROOMDETAILS_EN}


	#Label: ดูข้อมูลเจ้าของห้อง
	Wait Until Contains Element Text Should Be    ${ROOMMANAGEMENT_SEARCH_LBL_VIEWOWNERINFORMATION_LOCATOR}    ${ROOMMANAGEMENT_SEARCH_LBL_VIEWOWNERINFORMATION_EN}
...
Verify Action : click Create Room Owner button
	...
	Wait Until Contains Element Text Should Be    ${ROOMMANAGEMENT_SEARCH_LBL_HEADERPOPUPCREATE_ROOMA102_LOCATOR}    ${ROOMMANAGEMENT_SEARCH_LBL_HEADERPOPUPCREATE_ROOMA102_EN}
...
Verify Action : click View owner information button
	...
	Wait Until Contains Element Text Should Be    ${ROOMMANAGEMENT_SEARCH_LBL_HEADERPOPUPVIEW_OWNERINFORMATION_LOCATOR}    ${ROOMMANAGEMENT_SEARCH_LBL_HEADERPOPUPVIEW_OWNERINFORMATION_EN}
