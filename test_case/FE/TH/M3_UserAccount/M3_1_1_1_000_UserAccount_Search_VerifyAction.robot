...
Verify API Onload and mapping data after click User Account
	...
	${checkNodata}=    Run Keyword If    '${rowcountSearchUserAccount}'=='0'    Wait Until Contains Element Text Should Be    ${USERACCOUNT_SEARCH_LBL_RESULT_NODATA_LOCATOR}    ${DEFAULT_LBL_NODATA_TH}    #Nodata
...
Verify Action : click search button (Input data field Role and click search)
	Wait Until Contains Input Text    ${USERACCOUNT_SEARCH_TXT_SEARCHFILTER_AUTOCOMPLETE_LOCATOR}   ${USERACCOUNT_ROLE_TH_VALUE1}
	Sleep    0.5s
	#Click Search
	Click Element Page    ${USERACCOUNT_BTN_SEARCH_LOCATOR}
	...
	#====== Verify DB ==========
    #Inquiry for verify DB  
    ${resultSearchUserAccount}=    Run keyword And Continue On Failure    Request Verify DB Check Data Search Check User Account    ${MYSQL_TYPE_USERACCOUNT_SEARCH_ROLENAME_TH}   ${LIMIT_20}    ${OFFSET_0}    ${DB_TB_USERACCOUNT_ORDERBY_CREATEAT_AND_UPDATEAT_DESC}
	Log To Console    resultSearchUserAccount::${resultSearchUserAccount}    
	Log    resultSearchUserAccount::${resultSearchUserAccount} 
...
Verify Action : click search button (Select data field Property Manager dropdown and click search )
	...
	#Property Manager
	Select Dropdown    ${USERACCOUNT_SEARCH_DDL_PROPERTYMANAGER_LOCATOR}    ${USERACCOUNT_SEARCH_DDL_PROPERTYMANAGER_LIST_LOCATOR}    ${USERACCOUNT_SEARCH_DDL_PROPERTYMANAGER_CLASS_LOCATOR}    ${USERACCOUNT_SEARCH_TH_PROPERTYMANAGER_VALUE2}
	Sleep    0.5s
	#Click Search
	Click Element Page    ${USERACCOUNT_BTN_SEARCH_LOCATOR}
	...
	#====== Verify DB ==========
    #Inquiry for verify DB  
    ${resultSearchUserAccount}=    Run keyword And Continue On Failure    Request Verify DB Check Data Search Check User Account    ${MYSQL_TYPE_USERACCOUNT_SEARCH_PROPERTIES_TH}   ${LIMIT_20}    ${OFFSET_0}    ${DB_TB_USERACCOUNT_ORDERBY_CREATEAT_AND_UPDATEAT_DESC}
	Log To Console    resultSearchUserAccount::${resultSearchUserAccount}    
	Log    resultSearchUserAccount::${resultSearchUserAccount} 
